...
export default function AdminDashboard() {
  ...
  const getUserCountByRole = async (userType) => {
    try {
      const response = await fetch("http://localhost:3001/count/users", {
        method: "POST",
        headers: {
  ...
  const getAppointmentCount = async () => {
    try {
      const response = await fetch("http://localhost:3001/count/appointments", {
        headers: {
          authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
      ...
      if (responseData.totalAppointments) {
        setAppsTodayCount(responseData.totalAppointments);
      }

      if (responseData.pendingAppointments) {
        setPendingAppsTodayCount(responseData.pendingAppointments);
      }
    } catch (error) {
  ...
  const getBookedSlots = async () => {
    try {
      ...
      if (responseData.message === "success") {
        const aptms = responseData.appointments;
        console.log("aptms", aptms);
        setBookedAppointments(aptms);
      }
    } catch (error) {
